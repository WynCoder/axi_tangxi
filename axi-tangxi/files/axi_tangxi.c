...
void s2mm_ps_data_mm2s_pl(uint32_t rx_data_ps_addr, uint32_t tx_data_pl_addr,
                          uint32_t burst_size) {
  // DMA_MM2S_PL_ADDR
  writel(tx_data_pl_addr << 4, dma_mm2s_pl_addr);
  // DMA_MM2S_PL_SIZE
...
void batch_mm2s_ps_data_tangxi(struct axitangxi_transfer *axitangxi_trans) {
  ...
  while (data_remaining > 0) {
    ...
    count++;
    axitangxi_info(
    "PS DDR --> tangxi: 第 %d 次突发，突发长度：%d, 剩下数据：%dB\n", count,
    burst_size, data_remaining);
    // 等待一次传输完成
    timeout = msecs_to_jiffies(AXI_TANGXI_TIMEOUT);
    // wait S2MM_PL_DONE
 ...
 * @return int
 */
int axitangxi_psddr_tangxi_loopback(struct axitangxi_device *dev,
                                   struct axitangxi_transaction *trans) {
  ...
  axitx_trans.rx_data_ps_ptr =
      axitangxi_uservirt_to_phys(dev, trans->rx_data_ps_ptr);
  axitx_trans.tx_data_pl_ptr = axitx_trans.rx_data_pl_ptr = 0x10000;
  axitx_trans.tx_data_ps_ptr =
          axitangxi_uservirt_to_phys(dev, trans->tx_data_ps_ptr);
  // axitx_trans.rx_data_pl_ptr = axitx_trans.rx_data_ps_ptr;
  axitx_trans.node = trans->node;

  printk("tx_data_ps_ptr: %#x, rx_data_ps_ptr: %#x, tx_data_pl_ptr: %#x, rx_data_pl_ptr: %#x, burst_size: %zd, burst_data: %zd\n", axitx_trans.tx_data_ps_ptr, axitx_trans.rx_data_ps_ptr, axitx_trans.tx_data_pl_ptr, axitx_trans.rx_data_pl_ptr, axitx_trans.burst_size, axitx_trans.burst_data);
  int i;
  ...
  // PS DDR --> PL DDR
  batch_mm2s_ps_data_tangxi(&axitx_trans);
